...
const mapWordPressTrainer = (wpTrainer: any): TrainerUser => {
  ...
  return {
    ...
    teachingStyle: '',
    // Handle specialties as string (comma-separated) or array
    specialties: Array.isArray(wpTrainer.specialties)
      ? wpTrainer.specialties
      : (typeof wpTrainer.specialties === 'string'
          ? wpTrainer.specialties.split(',').map((s: string) => s.trim()).filter(Boolean)
          : []),
